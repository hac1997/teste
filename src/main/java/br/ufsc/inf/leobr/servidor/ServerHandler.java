...
public class ServerHandler extends AbstractServerRpcHandler {
	...
	public ServerHandler(DualRpcServerDispatcher dispatcher) {
		super(dispatcher);
		// O objeto de configura??o ? o Server
		this.server = (Server) this.getServerHandlerConfigurationObject();

		logger.info("Objeto de configura??o ?: "
	...
	public DadosAplicacao estabelecerSessao(Long clientSessionId, Long idJogo,
		...
		logger.info("Server: Id de sess?o " + this.serverSessionId
				+ " corresponds to client session id: " + this.clientSessionId);

		DadosAplicacao appData = new DadosAplicacao();
		appData.serverSessionId = this.serverSessionId;

		this.nomeJogador = nomeJogador;
		this.jogo = server.cadastrarNoJogo(this, idJogo);

		logger.info("Jogador("+this.nomeJogador+"): Sess?o entre cliente e servidor estabelecida.");
		return appData;
	}
	...
	/**
	 * O jogador continua conectado mas a partida ? finalizada.
	 */
	...
	/**
	 * Obt?m o nome dos jogadores da partida na qual o jogador est? participando no momento
	 * @return
	 */
	...
	public void enviaMensagem(String s) {
		logger.info("Jogador("+this.nomeJogador+"): Invocando o m?todo receberMensagem no jogador");
		try {
	...
	public void recebeJogada(MarshalledObject jogada) {
		logger.info("Jogador("+this.nomeJogador+"): Invocando o m?todo receberJogada no jogador");
		try {
			// callAsync because no return needed from client
			this.getDispatcher().callAsync(PRIMARY_CLIENT_HANDLER_CLASSNAME,
					"recebeJogada", jogada);
		} catch (Exception ex) {
			// se n?o ? possivel enviar uma jogada ent?o deve finalizar a
			// partida.
	...
	public void iniciarNovaPartida(Integer posicao) {
		logger.info("Jogador("+this.nomeJogador+"): Invocando o m?todo iniciarNovaPartida no jogador");
		try {
			// callAsync because no return needed from client
			this.getDispatcher().callAsync(PRIMARY_CLIENT_HANDLER_CLASSNAME,
	...
	public void finalizarPartidaComErro(String message) {
		logger.info("Jogador("+this.nomeJogador+"): Invocando o m?todo finalizarPartidaComErro no jogador");
		try {
			this.partida = null;
			this.getDispatcher().callAsync(PRIMARY_CLIENT_HANDLER_CLASSNAME,
	...
	public void tratarPartidaNaoIniciada(String message) {
		logger.info("Jogador("+this.nomeJogador+"): Invocando o m?todo tratarPartidaNaoIniciada no jogador");
		try {
			this.getDispatcher().callAsync(PRIMARY_CLIENT_HANDLER_CLASSNAME,
