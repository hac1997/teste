...
public class Server {
	...
	public void listen() throws IOException, ClassNotFoundException {
		...
		if (in == null) {
			logger.info("Server: Arquivo nao encontrado internamente.");
			in = new BufferedInputStream(new FileInputStream(
					arquivoMultiplayer));

			if (in == null) {
				logger.info("Server: Arquivo de configuracao nao encontrado externamente.");
			}
	 ...
	 * Caso o jogo n?o exista ele cria a representa??o do jogo.
	 * @param jogador
	 * @param idJogo
	 * @return
	 */
	public synchronized Jogo cadastrarNoJogo(ServerHandler jogador, Long idJogo) {
		...
		synchronized (this.jogos) {
			if (jogos.containsKey(idJogo)) {
				logger
				.info("Server: Jogo " + idJogo+ "j? est? rodando.");
				jogo = jogos.get(idJogo);
			} else {
				jogo = new Jogo(idJogo);
				jogos.put(idJogo, jogo);
				logger.info("Server: Jogo "+ idJogo+ " criado. N?mero de jogos ativos: " + this.jogos.size()); 
	 ...
	 * Confere se o jogo n?o possui mais jogadores e somente depois retira ele da lista de jogos ativos.
	 * @param jogo
	 */
	public synchronized void removerJogo(Jogo jogo) {
		
		logger.info("Server: Verificando se ? poss?vel finalizar o jogo " + jogo.getIdJogo()+ " no servidor"); 
		if (jogo.estahFinalizado()) {
			if (this.jogos.containsValue(jogo)) {
				jogo.finalizar();
				this.jogos.remove(jogo.getIdJogo());
				logger.info("Server: Jogo "+jogo.getIdJogo()+" removido do servidor. N?mero de jogos ativos: "+ this.jogos.size());
			}
