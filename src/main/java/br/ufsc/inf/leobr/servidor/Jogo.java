...
public class Jogo {
	 ...
	 *            O n?mero exato de jogadores que essa partida deve conter,
	 *            incluindo o jogador que fez a requisi??o
	 */
		...
		synchronized (this.jogadores) {
			...
			if (quantidadeJogadores.equals(jogadoresPartida.size())) {
				...
				synchronized (this.partidas) {
					...
					logger.info("Jogo("+ idJogo +"): Uma nova partida adicionada. N?mero de partidas: " +partidas.size());
				}
				
				partida.iniciaPartida(jogadorPedinte);
			}else{
				jogadorPedinte
				.tratarPartidaNaoIniciada("N?o haviam jogadores sulficientes no servidor. Por favor tente novamente mais tarde");
				logger.info("Jogo("+ idJogo +"): A partida n?o pode ser iniciada porque n?o haviam jogadores suficientes.");
			}
	...
	/**
	 * Remove um jogador do jogo. Se o jogador n?o estiver no jogo, procura em
	 * uma das partidas do jogo. Se encontrado ent?o remove o jogador da
	 * partida.
	 * 
	 * @param jogador
	 */
	public void removerJogador(ServerHandler jogador) {
		...
		if (jogadores.contains(jogador)) {
			...
			logger.info("O jogador "+  jogador.getNomeJogador() +" n?o estava presente no jogo " +idJogo +".");
		}
	...
	public void encerrarPartida(Partida partida) {
		...
		synchronized (this.partidas) {
			if (this.partidas.contains(partida)){
				...
				logger.info("Jogo("+ idJogo +"): Partida removida. N?mero de partidas: " + partidas.size());
			}else{
				logger.info("Jogo("+ idJogo +"): A partida"+ partida.getIdPartida()+" n?o est? nesse jogo");
			}
		}
